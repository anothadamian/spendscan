...
(async()=>{
 ...
 await check('forwarded browser host survives Next.js URL normalization',async()=>{const r=await request();r.headers.set('host','127.0.0.1:3000');r.headers.set('origin','http://127.0.0.1:3000');assert.equal((await route().POST(r)).status,200);});
